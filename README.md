...
### 绘画
- **笔刷栏 22 支笔**：铅笔、圆笔 19（PS 经典的 19 号圆头笔）、勾线笔、毛发笔、干笔、粉笔、加色笔、
  软铅笔、喷枪、画笔、水彩笔、马克笔、橡皮擦、油漆桶、渐变、模糊、特效笔、散布、涂抹，
  外加 **草稿铅笔 / 水彩（渗化）/ 油画厚涂** 三支
- **12 支笔的参数直接取自 SAI2 笔刷包**（SAI2-CATXQ）：铅笔←2B、喷枪、画笔、水彩笔、马克笔、
  橡皮擦、选区笔、选区擦、模糊、特效笔、散布、涂抹，以及那三支新增的 —— 硬度 / 最小直径 /
  笔压曲线 / 颗粒 / 混色都按 SAI2 的真实参数换算过来（笔尖尺寸仍用茶绘的像素口径）
- **导入 Photoshop / Clip Studio Paint / Procreate / SAI2 / 画世界Pro 的笔刷**：
  直接吃 `.abr`、`.sut`、`.brush`、`.brushset`、`.saitdat`（SAI2）、`.bru`，
  把笔尖形状和相关参数搬进来（见下面「导入笔刷」）
...
- **套索和 PS 一样**：拖的时候只画那条线，**松手才闭合成圈**
- **魔棒有一整块选项面板**（照 SAI2）：取样模式（**被线条包围的透明区域** / 色差范围内的区域 /
  色差范围内的**全部像素**）、透明容差范围、防止溢出范围（把选区往外长 N 像素）、
  取样来源（当前图层 / **指定为选区样本的图层** / 拼合图像）、消除锯齿、忽略已选择的区域
- `Shift` 加选、`Alt` 减选，普通模式为「替换」
- 蚂蚁线**沿着选区的真实形状**走（套索套个圈就是圈的虚线，不是包围盒方框）；
  有选区时画笔只在选区内生效，画到区外会明确提示而不是「静默没反应」
...
- 图层操作都在**图层列表上面**两排**图标按钮**里（每颗都有悬停提示）：新建 / 新建组 / 复制 /
  清除 / 删除 / 向下合并 / **内容转移到下层**（把这一层的内容原样倒进下一层，不施加本层的不透明度与混合模式）/
  合并可见 / 进-出组 / 蒙版（新建 / 编辑 / 删除）
- **锁定行**（照 SAI2，四颗图标）：锁定透明像素 / 锁定画笔 / 锁定移动 / 全部锁定，
  外加一行「创建剪贴蒙版」和一颗「**指定为选区样本**」（魔棒的取样来源要用它）
- **图层组**：把几层打包成一个「组」，整组一起调**不透明度 / 混合模式**（17 种都行）。
  ...
  **上下顺序靠拖着排**（按住一行上下拖，落点画一条横线）：整组连着一起挪；
  拖进某个展开的组那一块就归到那个组、拖到组外自动脱离 —— 已经没有「上移 / 下移」按钮了。
...
> | 接龙 | 主题词库 · 作画时间 · **玩法（接龙 / 传词接龙）** · 链长 · **传词轮数** · **写词时间** · **猜词时间** · **回放时间** · **投票时间** |
...
- **两种玩法**（开局面板里选）：
  - **接龙**（默认）：猜完**自己画**自己猜出来的词，再传给下家猜。4 人时每人连着两格，
    链长 = **2 × 组人数**。
  - **传词接龙**：猜完**不画**，把猜出来的词直接交给**下家画**，画完再交给再下家猜 ——
    每人每轮只出一格，链长 = **1 + 传几轮 × 组人数**（轮数 1～4 可选，默认 2）。
    同一条词一路「画 → 猜 → 画 → 猜」地传下去，更像传话游戏。
- **回放是逐笔长出来的**：每一笔都有自己的时间片 —— 这一格时间宽裕就一笔一笔慢慢长
  （≈每笔 240ms），时间紧就整段**自适应倍速**播完；到最后也不会「前面慢慢来、
  啪一下跳到成图」。
...
### 导入笔刷（Photoshop `.abr` / CSP `.sut` / Procreate `.brush` / SAI2 / 画世界Pro）
笔刷栏标题上的 **「导入」** 按钮，选一个或多个 `.abr` / `.sut` / `.brush` / `.brushset` /
`.saitdat` / `.saitset` / `.bru` 文件，
...
- **Procreate `.brush` / `.brushset`**：`.brush` 是单支、`.brushset` 是一套（每支笔住一个子目录）。
  ...
  不会被误判成一坨软边。
- **SAI2 `.saitdat` / `.saitset`**：就是纯文本的 `key=value`，值前面带一个类型标签
  （`I:` 数字 / `S:` / `U:` 字符串）。笔名取文件里的 `name`（不是文件名）；
  选区笔 / 选区擦 / 油漆桶这类**取色工具会被明确拒绝**（它们不是笔刷）。
  笔尖形状是**同目录的一张 BMP** —— `fomnam` 写的就是那张图的名字，`fomcat` 说它是笔尖还是纹理。
  所以导入 SAI2 笔刷时**把它和那张 BMP 一起选**（BMP 自己不会被当成第二支笔）；
  没配到素材就按硬度合成一个圆笔尖，照常能用。
- **画世界Pro `.bru`**：**这个格式没有公开的文档**，所以这里走「多容器试探」——
  开头是 `.abr` 的版本字就当 `.abr` 读（社区最常见的做法就是把 `.abr` 改名成 `.bru`）、
  是 ZIP 包就从里面找最大的那张图当笔尖、裸容器就按 PNG 签名扫。
  三条路都认不出来会**明确报错**并给出去路（把后缀改回 `.abr`，或直接把笔尖图当普通图片铺到画布上）。
...
> 定义里写着 `selpen` 这类选区工具的 `.saitdat`、认不出容器的 `.bru`，
> 都会弹一句人话，不会静默给你一支错的笔。
>
...
> SAI2 也从本机安装里取了 3 个**真实** `.saitdat`（含配套的笔尖形状 BMP）放进
> `tools/sai-corpus/`，同样由 `npm run test:import` 走「导入」按钮验一遍。
...
- **颜色栏**（对照 SAI2）：
  ...
  - 色轮：色环取色相，里面的取色区取饱和度 / 明度。取色区**默认是方形**
    （横轴饱和度、纵轴明度，和 PS / SAI 一致 —— 三角尖角附近一片颜色挤在一起，不好瞄准），
    色轮右上角那颗小按钮可以切回**三角形**，选择记在本地。
    按住 `Shift` 色相每 15° 吸一档。点取色区绝不会改到色相；
    取色区边界往外偏一两个像素仍然算取色区（不会误跳到色环），两者之间也没有「点了没反应」的死角
...
npm run test:chain-flow # 接龙 UI 全流程（真浏览器：写词→画→猜→回放→投票→奖杯）
# ⚠ tools/test-chain-ui.js 已经**作废**（写的是「回放用全屏遮罩弹窗」那套旧界面，
#   那些 DOM 早删了）。要跑接龙 UI 就用上面这条；`test:chain-ui` 现在也指向它。
...
npm run test:overlay   # 覆盖层回归（画布上不该有多余的方框和大椭圆；
                       #   套索拖拽中只有那条线、松手才成圈；选区轮廓沿真实形状——包围盒角上不许有虚线）
npm run test:wand      # 魔棒选项面板回归（三种取样模式 / 透明容差 / 防止溢出 / 取样来源 / 抗锯齿 / 忽略已选）
npm run test:chain-anim # 接龙逐笔回放的节奏（窗口紧时自适应倍速播完，不会「啪的一下跳到成图」）
...
npm run test:wheel    # 色轮取色回归（方形 / 三角形两种形状的取色与指示器、切形状记本地）
npm run test:alt-picker # Alt 临时吸管（按住 Alt 光标变吸管 / Alt+拖是取色不落笔 / 选区工具下 Alt 不被抢走）
