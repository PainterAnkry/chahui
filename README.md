...
房间里点顶栏的 **「游戏」**，房主可以就地开一局「你画我猜」——不用另开软件、不用切房间，玩完接着画。

- **怎么玩**：房主选回合数（默认 6 回）开局 → 系统轮着点名一个人当**画手**，
  给他 3 个词里挑一个（都不好画可以**换一组**，每回合 1 次）→ 他有 80 秒把它画出来 →
...
- **怎么玩**：每人先给自己的链**想一个词**（也有系统给的候选）→ 那个词连同链条**匿名传给下一个人**，
  他只能看图**猜一个词** → 猜出来的词再传给下一个人去**画**……一路传下去，
  每一圈换一个人接手，直到走完设定的圈数（默认 3 圈 ＝ 词→画→猜，最短的完整循环）。
- **结算「公开处刑」**：全部传完 → 所有人一起看**回放**（一条链一张卡，逐格摊开词/图）
  → 投票**「首尾对得上吗」**。首尾一致、且多数人不反对 → **起词的那个人拿一个奖杯**🏆。
  打完整局按奖杯数排名。
- **回放是逐格「翻」出来的**：一格一格揭晓（进度小点可以点，键盘 ←/→ 也能翻页），
  最后给出「传了 N 手」的首尾对照。整段动画由 JS 定时驱动，手动翻页会干净地取消重排，
  并尊重系统的「减弱动态效果」设置。
...
| 要求 | 做法 |
| --- | --- |
| **绝不能提前看到别人的词/画** | 题面走**单独一条私有消息** `game:task` 单发本人（`write`→候选词 / `draw`→要画的词 / `guess`→上家的图+字数）。快照 `snapshotFor()` 按收件人裁剪，**回放内容在投票开始前压根不下发** |
| **手里攥着答案的时候不能聊天** | 作画 / 猜词那两步的人发言会被服务端拦下并私信提醒 —— 否则一句剧透毁一条链 |
| **不是你的步骤就不能落笔** | 和「你画我猜」同一道锁：`canDraw()` 查 `lockedFor(userId)`，非当前步骤的人画布操作**直接被拒** |
| **有人掉线 / 挂机不能卡死** | 每步都有超时；超时按「空格子」跳过（下家会看到空白），房主还能手动推进 |
| **一轮不能卡在原地** | 换圈时清空画布并把 `seq` 水位一起推进（老坑：不抬水位就「画了看不见」） |

> **圈数 = 每条链传几手**。3 圈就有一次完整的「画→猜」；2 圈只有「词→画」，**没有猜词环节** ——
> 所以默认是 3，不是随便定的。
...
#### 入口页那颗「本机服务器」开关：在线 / 离线一个按钮切

入口弹窗的「服务器」下面有一行 **本机服务器**（只有桌面端有，网页版会整行藏起来）：

- 显示 **关闭服务器** → 服务器正开着，右侧小字写着局域网地址，点一下就把服务器**停掉**
- 显示 **开启服务器** → 服务器没开，点一下就把它**开起来**并把客户端连上去
- 菜单里也有一条 **其他 → 本机服务器**（前面带 ✓ 表示正开着）

这里要强调的是「关闭」的语义：**不是断开连接，是真的把服务器停掉** ——
不再监听任何端口，同机别的程序也连不进来。

那关掉之后还能画吗？能，靠的是**离线模式**：主进程里挂一个**不走 socket 的客户端**
...
| 通道 | 是什么 | 什么时候用 |
| --- | --- | --- |
| `ws://…` | 普通 WebSocket | 网页版、连别人的服务器、连公网 |
| `local://` | 进程内的假地址（**不占端口、不出网**） | 桌面端关掉服务器之后 |

所以「离线模式」在代码里就是一个 `local://` 的地址，别的什么都没变 ——
消息还是那些消息，只是不经过网络。关闭服务器会先把还连着的客户端剪掉
（`server.close()` 的回调要等所有连接自己断开，不剪的话界面会卡在「正在关闭…」），
房间存档不受影响；再开启时 WebSocket 服务端会**重建一个实例**
（`close()` 过的那个是终态，复用的话端口能连上但握手永远不成功）。

> 内置服务器也可以关掉不让它开机自启：配置文件 `<用户数据目录>/chahu-config.json` 里设
> `"embeddedServer": false`。不过日常不需要动配置 —— 用上面那颗按钮切就好。
...
├── client/
│   ├── main.js              Electron 主进程（启动时拉起内置服务器；「开启 / 关闭服务器」的 IPC）
...
- **接龙的传递规则是一条公式**：一条链第 k 格由 `order[(ownerIdx + k) % n]` 来做，
  k=0 起词、k 奇数作画、k 偶数猜词。**n ≥ 4 保证没人会接到自己的东西**，这也是为什么
  下限是 4 人（不是随便定的）。**2 圈只有「词→画」，没有猜词环节** —— 所以默认 3 圈。
...
npm run test:local     # 离线宿主回归（纯 Node：一个 socket 都不碰就能建房 / 落笔 / 图层 / 聊天，
                       #   两个会话互相看得见 —— 这是「关掉服务器还能画」唯一的支点）
npm run test:server-toggle # 「开启 / 关闭服务器」回归（纯 Node：关掉后端口真的连不进来、
                       #   存档不丢、**再开启时 wss 重建后新的 WebSocket 还能握手成功**）
npm run test:server-button # 入口页那颗服务器开关的界面回归（桌面端桥用打桩的，不用起 Electron：
                       #   网页版整行藏起来 / 文案与状态分开 / 关→切离线通道且状态栏写「离线模式」/
                       #   开→收掉离线会话并真的连回服务端）
