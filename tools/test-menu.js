...
const WANT = ['文件', '编辑', '图像', '选择', '尺子', '滤镜', '视图', '窗口', '其他'];
...
(async () => {
  ...
  console.log('\n=== 菜单结构 ===');
  const menus = await page.evaluate(() => Array.from(document.querySelectorAll('#menuBar .menu-title')).map(b => b.textContent));
  ok('九个菜单都在', WANT.every(w => menus.indexOf(w) >= 0), menus.join(' / '));
  ok('顺序就是要求的那九个', menus.join(',') === WANT.join(','), menus.join(','));

  const counts = await page.evaluate(() => {
    const out = {};
    document.querySelectorAll('#menuBar .menu-item').forEach(function (w) {
      out[w.querySelector('.menu-title').textContent] = w.querySelectorAll('.menu-drop .menu-row').length;
    });
    return out;
  });
  ...
  ok('菜单项总数够用（≥ 60）', total >= 60, total + ' 条');
  ...
  ok('有一批默认快捷键（≥ 20）', Object.keys(keys).length >= 20, Object.keys(keys).length + ' 条');
  ...
  ok('对话框能打开并列出全部动作', dlg.open && dlg.rows === total && dlg.inputs === total, JSON.stringify(dlg));
  ok('按菜单分组显示', dlg.groups === 9, String(dlg.groups));
  ...
  ok('「全部恢复默认」把改过的都还原',
    restored.flip === 'H' && restored.zoom100 === '1' && restored.zoomIn === '=', JSON.stringify(restored));
  ...
  for (const name of WANT) {
    const opened = await page.evaluate((n) => {
      const t = Array.from(document.querySelectorAll('#menuBar .menu-title')).find(x => x.textContent === n);
      t.click();
  ...
  await page.evaluate(() => {
    const t = Array.from(document.querySelectorAll('#menuBar .menu-title')).find(x => x.textContent === '选择');
    t.click();
  });
  await sleep(300);
